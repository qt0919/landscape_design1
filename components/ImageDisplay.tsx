...
const ImageCard: React.FC<{ image: ImageState | null; title: string; children?: React.ReactNode; isOriginal?: boolean; }> = ({ image, title, children, isOriginal }) => (
    <div className="w-full md:w-1/2 p-2">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden h-full">
            <h3 className="text-center font-bold text-lg py-2 bg-gray-50 text-gray-700">{title}</h3>
            <div className="aspect-w-16 aspect-h-9 relative">
                {image ? (
                    <img src={`data:${image.mimeType};base64,${image.base64}`} alt={isOriginal ? 'Original user-uploaded garden' : 'AI-generated garden design'} className="object-cover w-full h-full" />
                ) : (
...
const ImageDisplay: React.FC<ImageDisplayProps> = ({ originalImage, generatedImage, isLoading }) => {
  return (
    <div className="w-full flex flex-col md:flex-row">
      <ImageCard image={originalImage} title="Before" isOriginal={true} />
